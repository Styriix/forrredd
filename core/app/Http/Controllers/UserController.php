...
class UserController extends Controller
{
    ...
    public function getDashboard()
    {
        ...
        $data['trade'] = Purchase::where('user_id', Auth::user()->id)
    ...
    {
        $data['page_title'] = "Buy Product";
        $data['products'] = Product::orderBy('id', 'DESC')->where('status', 1)->where('category_id', $category_id)->get();
    ...
    {
        $data['page_title'] = "Buying Details";
    ...
    public function productBuyDetails(Request $request)
    {
        $data['page_title'] = "Buying Details";
        $data['product'] = Product::findorFail($request->product_id);
        return view('user.buy-product-details', $data);
    }

    public function productBuySubmit(Request $request)
    {
        $request->validate([
        ...
        if (round($user->balance, $basic_settings->deci) < round($product->price, $basic_settings->deci)) {
            session()->flash('alert','Insufficient Balance.');
            return redirect()->route('buy-product');
        }

        $requestInputs = Input::except('_token');
        $requestInputs['transaction_id'] = strtoupper(Str::random(20));
        Purchase::create($requestInputs);


        $user_log['user_id'] = $user->id;
        $user_log['amount'] = $request->amount;
        $user_log['charge'] = null;
        // amount type 17 refers to user bought product
        $user_log['amount_type'] = 17;
        $user_log['post_bal'] = $user->balance - $request->amount;
        $user_log['description'] = $product->name." purchaded for ".$request->amount." ".$basic_settings->currency;
        $user_log['transaction_id'] = $requestInputs['transaction_id'];
        UserLog::create($user_log);

        $user->balance = $user->balance -  $request->amount;
        $user->save();

        //user under reference gets bonus for the trade
        if ($user->under_reference != 0){
            $refMem = User::findOrFail($user->under_reference);
            $refAmo = round(($request->amount * $basic_settings->reference_percent) / 100, $basic_settings->deci);
            ...
            $ul['amount_type'] = 3;
            $ul['description'] = "Reference Trade Bonus ".$refAmo." ".$basic_settings->currency." . From - $user->username.";
            $ul['transaction_id'] = $requestInputs['transaction_id'];
            UserLog::create($ul);

            $refMem->balance = $refMem->balance + $refAmo;
            $refMem->save();

            if ($basic_settings->email_notify == 1){
            $text = $refAmo." - ". $basic_settings->currency ." Reference Trade Bonus From - $user->username. <br> Transaction ID Is : <b>#".$requestInputs['transaction_id']."</b>";
            $this->sendMail($refMem->email,$refMem->name,'Reference Trade Bonus.',$text);
            }

            if ($basic_settings->phone_notify == 1){
            $text = $refAmo." - ".$basic_settings->currency ." Reference Trade Bonus From - $user->username.. <br> Transaction ID Is : <b>#".$requestInputs['transaction_id']."</b>";
    ...
    {
        $data['page_title'] = "Puchase History";
        $data['purchase_histories'] = Purchase::orderBy('id', 'DESC')->where('user_id', Auth::user()->id)->get();
        return view('user.purchase-history', $data);
    }
    
    public function tradeHistory()
    {
        $data['page_title'] = "Trade History";
        $data['trade_histories'] = UserTradeLog::orderBy('id', 'DESC')->where('user_id', Auth::user()->id)->with('trade')->get();
